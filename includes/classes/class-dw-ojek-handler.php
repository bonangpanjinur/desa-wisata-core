...
class DW_Ojek_Handler {
    ...
    public function toggle_driver_status() {
        ...
        if ($new_status === 'active') {
            if (!$this->can_driver_activate($user_id)) {
                ...
                // Helper format price if available, otherwise manual format
    ...
    private function can_driver_activate($user_id) {
        ...
        // Asumsi ada class DW_Wallet atau fungsi helper dw_get_wallet_balance
        // Jika belum ada, kita ambil manual dari user meta '_dw_wallet_balance'
        $current_balance = (int) get_user_meta($user_id, '_dw_wallet_balance', true);
